...
小鹅通允许商家把店铺挂在自己的域名下，各家策略不一样，域名没法穷举。
识别不靠域名白名单，而是分两层兜底：

1. 小鹅通给自有域名统一加的 `xet-pc.` / `.xet.` 前缀，例如 `app5xxx.xet-pc.citv.cn`
2. 连前缀都没有时，看 URL 路径特征（`/p/t_pc/`、`course_pc_detail`、`v_`/`l_` 资源 id）

所以换一家店铺、换个域名通常不用改代码。真遇到认不出来的，
`./vd doctor "<链接>"` 会直接告诉你平台和编码有没有识别成功。
